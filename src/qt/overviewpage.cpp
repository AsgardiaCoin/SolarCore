...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ASL)
    {
    }
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnableObfuscation) {
                ui->toggleObfuscation->setText(tr("Start DarkSend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop DarkSend"));
            }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ASL")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSolarAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSolarAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSolarAmount = strAnonymizeSolarAmount.remove(strAnonymizeSolarAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSolarAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSolarAmount * COIN) nMaxToAnonymize = nAnonymizeSolarAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSolarAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSolarAmount));
        strAnonymizeSolarAmount = strAnonymizeSolarAmount.remove(strAnonymizeSolarAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSolarAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
