...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "Solar.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "Solard.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 48433, 48455));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ASL/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ASL/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-rescan", _("Rescan the block chain for missing wallet transactions") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-salvagewallet", _("Attempt to recover private keys from a corrupt solar.dat") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-wallet=<file>", _("Specify wallet file (within data directory)") + " " + strprintf(_("(default: %s)"), "solar.dat"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, Solar, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ASL/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Solar specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:48433"));
    ...
    strUsage += HelpMessageOpt("-anonymizeSolaramount=<n>", strprintf(_("Keep N ASL anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 48434, 39799));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The Solar Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Solar is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    std::string strWalletFile = GetArg("-wallet", "solar.dat");
#endif // ENABLE_WALLET
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Solar Core is shutting down."));
...
#endif
    // Make sure only a single Solar process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Solar Core is probably already running."), strDataDir));
    ...
    if (!fDisableWallet) {
            ...
            if (filesystem::exists(backupDir)) {
                ...
                for (boost::filesystem::directory_iterator dir_iter(backupFolder); dir_iter != end_iter; ++dir_iter) {
                    ...
                    if (boost::filesystem::is_regular_file(dir_iter->status())) {
                        ...
                        // Only add the backups for the current wallet, e.g. solar.dat.*
        ...
        if (filesystem::exists(GetDataDir() / strWalletFile)) {
            ...
            if (r == CDBEnv::RECOVER_OK) {
                string msg = strprintf(_("Warning: solar.dat corrupt, data salvaged!"
                                         " Original solar.dat saved as wallet.{timestamp}.bak in %s; if"
            ...
            if (r == CDBEnv::RECOVER_FAIL)
                return InitError(_("solar.dat corrupt, salvage failed"));
        }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-zapwallettxes", false)) {
            ...
            if (nZapWalletRet != DB_LOAD_OK) {
                uiInterface.InitMessage(_("Error loading solar.dat: Wallet corrupted"));
                return false;
            }
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                strErrors << _("Error loading solar.dat: Wallet corrupted") << "\n";
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR) {
                string msg(_("Warning: error reading solar.dat! All keys read correctly, but transaction data"
                             " or address book entries might be missing or incorrect."));
                InitWarning(msg);
            } else if (nLoadWalletRet == DB_TOO_NEW)
                strErrors << _("Error loading solar.dat: Wallet requires newer version of Solar Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Solar Core to complete") << "\n";
                LogPrintf("%s", strErrors.str());
                return InitError(strErrors.str());
            } else
                strErrors << _("Error loading solar.dat") << "\n";
        }
    ...
    nAnonymizeSolarAmount = GetArg("-anonymizeSolaramount", 0);
    if (nAnonymizeSolarAmount > 999999) nAnonymizeSolarAmount = 999999;
    if (nAnonymizeSolarAmount < 2) nAnonymizeSolarAmount = 2;
    ...
    LogPrintf("Anonymize Solar Amount %d\n", nAnonymizeSolarAmount);
